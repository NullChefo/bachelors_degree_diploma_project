...
import com.nullchefo.authorizationservice.domain.Authorization;
import com.nullchefo.authorizationservice.repository.AuthorizationRepository;
...
public class JpaOAuth2AuthorizationService implements OAuth2AuthorizationService {
    ...
    public OAuth2Authorization findByToken(String token, OAuth2TokenType tokenType) {
        ...
        if (tokenType == null) {
            result = this.authorizationRepository.findByStateOrAuthorizationCodeValueOrAccessTokenValueOrRefreshTokenValue(
                    token);
    ...
    private OAuth2Authorization toObject(Authorization entity) {
        ...
        OAuth2Authorization.Builder builder = OAuth2Authorization.withRegisteredClient(registeredClient)
                ...
                .authorizedScopes(StringUtils.commaDelimitedListToSet(
                        entity.getAuthorizedScopes()))
                .attributes(attributes -> attributes.putAll(parseMap(
    ...
    private Authorization toEntity(OAuth2Authorization authorization) {
        ...
        //		if (authorization.getAttributes().containsKey("java.security.Principal")) {
        //			Thread.ofVirtual().start(() -> {
        //				sendLoginEmail(authorization);
        //			});
        //		}
    ...
    private void sendLoginEmail(final OAuth2Authorization authorization) {
        // TODO send email
        var principle = (Principal) authorization.getAttributes().get("java.security.Principal");
        var usernamePasswordAuthenticationToken = (UsernamePasswordAuthenticationToken) principle;
        var webAuthenticationDetails = (WebAuthenticationDetails) usernamePasswordAuthenticationToken.getDetails();
